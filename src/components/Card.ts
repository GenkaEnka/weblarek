...
export class StoreItemPreview extends Card {
  ...
  // Переопределяем сеттер selected для изменения текста кнопки
  set selected(value: boolean) {
    if (!this._button) return;
    if (this._button.disabled && !value) return; // если цена null, кнопка недоступна
    if (value) {
  ...
  // Переопределяем сеттер price для блокировки при отсутствии цены
  set price(value: number | null) {
    this._price.textContent = value
      ? handlePrice(value) + ' синапсов'
      : 'Бесценно';
